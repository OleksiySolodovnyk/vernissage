...
function wpaq_options_page() {
	...
	$field2  = __('Атрибуты кнопки', FB_WPAQ_TEXTDOMAIN );
	...
	$export1 = __('Экспорт/Импорт настроек', FB_WPAQ_TEXTDOMAIN );
	...
	?>
	<div class="wrap">
		<h2><?php _e('Управление дополнительными кнопками', FB_WPAQ_TEXTDOMAIN ); ?></h2>
...
if (strpos($_SERVER['REQUEST_URI'], 'admin.php?page=events') || strpos($_SERVER['REQUEST_URI'], 'post.php') || strpos($_SERVER['REQUEST_URI'], 'post-new.php') || strpos($_SERVER['REQUEST_URI'], 'page-new.php') || strpos($_SERVER['REQUEST_URI'], 'page.php') || strpos($_SERVER['REQUEST_URI'], 'comment.php')) {
...
function wpaq_add_settings_page() {
	...
	if ( function_exists('add_options_page') && current_user_can('manage_options') ) {
		...
		$menutitle .= __('Дополнительные кнопки', FB_WPAQ_TEXTDOMAIN );
